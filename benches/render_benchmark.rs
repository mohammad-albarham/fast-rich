...
fn bench_markup_parsing(c: &mut Criterion) {
    ...
    c.bench_function("markup parsing", |b: &mut criterion::Bencher| b.iter(|| {
        markup::parse(black_box(&markup))
    }));
}
...
fn bench_text_rendering(c: &mut Criterion) {
    ...
    c.bench_function("text render", |b: &mut criterion::Bencher| b.iter(|| {
        text.render(black_box(&context))
    }));
}
...
fn bench_table_rendering(c: &mut Criterion) {
    ...
    c.bench_function("table render (100 rows)", |b: &mut criterion::Bencher| b.iter(|| {
        table.render(black_box(&context))
    }));
}
...
criterion_group!(benches, bench_markup_parsing, bench_text_rendering, bench_table_rendering);
